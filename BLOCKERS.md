...
- The Scan tab is now structurally improved, but native camera/barcode/OCR behavior is still not rebuilt to the level described in the product spec.
...
- Which major flow should be prioritized immediately after this first scan-hub rebuild (native barcode camera, OCR/manual analysis path, leaderboard/auth, history sync, etc.)
